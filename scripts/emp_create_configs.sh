...
# Webserver assets path prefix.
# Mandatory.
echo -n "Give webserver assets path prefix [$EMP_WEBSERVER_ASSETS_PATH_PREFIX]: "
read TEMP_INPUT

if [ -z "$TEMP_INPUT" ]
then
    if [ -z "$EMP_WEBSERVER_ASSETS_PATH_PREFIX" ]
    then
	echo "ERROR: No webserver assetsh path prefix given, exiting."

	exit 1
    fi
else
    EMP_WEBSERVER_ASSETS_PATH_PREFIX="$TEMP_INPUT"
fi
...
echo "WEBSERVER_ASSETS_PATH_PREFIX=$EMP_WEBSERVER_ASSETS_PATH_PREFIX" >> "$EMP_MAIN_CONFIG"
...
sed "s|{EMP_DNSMASQ_CONF_FILE}|$EMP_DNSMASQ_CONF_FINAL|g;s|{EMP_TFTPROOT_DIR}|$EMP_TFTPROOT_DIR|g;s|{EMP_WEBSERVER_PROTOCOL}|$EMP_WEBSERVER_PROTOCOL|g;s|{EMP_WEBSERVER_IP}|$EMP_WEBSERVER_IP|g;s|{EMP_WEBSERVER_ASSETS_PATH_PREFIX}|$EMP_WEBSERVER_ASSETS_PATH_PREFIX|g;s|{EMP_DRIVERS_BASE_DIR}|$EMP_DRIVERS_BASE_DIR|g;s|{EMP_WEBSERVER_DRIVERS_PATH_PREFIX}|$EMP_WEBSERVER_DRIVERS_PATH_PREFIX|g;" "$EMP_DNSMASQ_CONF_TEMPLATE" > "$EMP_DNSMASQ_CONF_FINAL"
echo "done."


echo -n "Writing $EMP_APACHE_CONF_FINAL ... " 
sed "s|{EMP_CONFIG_DIR}|$EMP_CONFIG_DIR|g;s|{EMP_WEBSERVER_ASSETS_PATH_PREFIX}|$EMP_WEBSERVER_ASSETS_PATH_PREFIX|g;s|{EMP_SCRIPTS_DIR}|$EMP_SCRIPTS_DIR|g;s|{EMP_ASSETS_ROOT_DIR}|$EMP_ASSETS_ROOT_DIR|g;s|{EMP_TFTPROOT_DIR}|$EMP_TFTPROOT_DIR|g;s|{EMP_DRIVERS_BASE_DIR}|$EMP_DRIVERS_BASE_DIR|g;s|{EMP_WEBSERVER_DRIVERS_PATH_PREFIX}|$EMP_WEBSERVER_DRIVERS_PATH_PREFIX|g;" "$EMP_APACHE_BASE_CONF_TEMPLATE" > "$EMP_APACHE_CONF_FINAL"

if [ -n "$EMP_DRIVERS_BASE_DIR" ]
then
    sed "s|{EMP_CONFIG_DIR}|$EMP_CONFIG_DIR|g;s|{EMP_WEBSERVER_ASSETS_PATH_PREFIX}|$EMP_WEBSERVER_ASSETS_PATH_PREFIX|g;s|{EMP_SCRIPTS_DIR}|$EMP_SCRIPTS_DIR|g;s|{EMP_ASSETS_ROOT_DIR}|$EMP_ASSETS_ROOT_DIR|g;s|{EMP_TFTPROOT_DIR}|$EMP_TFTPROOT_DIR|g;s|{EMP_DRIVERS_BASE_DIR}|$EMP_DRIVERS_BASE_DIR|g;s|{EMP_WEBSERVER_DRIVERS_PATH_PREFIX}|$EMP_WEBSERVER_DRIVERS_PATH_PREFIX|g;" "$EMP_APACHE_DRIVERS_CONF_TEMPLATE" >> "$EMP_APACHE_CONF_FINAL"
fi
echo "done."


echo -n "Writing $EMP_NGINX_CONF_FINAL ... " 
sed "s|{EMP_CONFIG_DIR}|$EMP_CONFIG_DIR|g;s|{EMP_WEBSERVER_ASSETS_PATH_PREFIX}|$EMP_WEBSERVER_ASSETS_PATH_PREFIX|g;s|{EMP_SCRIPTS_DIR}|$EMP_SCRIPTS_DIR|g;s|{EMP_ASSETS_ROOT_DIR}|$EMP_ASSETS_ROOT_DIR|g;s|{EMP_TFTPROOT_DIR}|$EMP_TFTPROOT_DIR|g;s|{EMP_PHP_FPM_RUN_SOCK}|$EMP_PHP_FPM_RUN_SOCK|g;s|{EMP_DRIVERS_BASE_DIR}|$EMP_DRIVERS_BASE_DIR|g;s|{EMP_WEBSERVER_DRIVERS_PATH_PREFIX}|$EMP_WEBSERVER_DRIVERS_PATH_PREFIX|g;" "$EMP_NGINX_BASE_CONF_TEMPLATE" > "$EMP_NGINX_CONF_FINAL"

if [ -n "$EMP_DRIVERS_BASE_DIR" ]
then
    sed "s|{EMP_CONFIG_DIR}|$EMP_CONFIG_DIR|g;s|{EMP_WEBSERVER_ASSETS_PATH_PREFIX}|$EMP_WEBSERVER_ASSETS_PATH_PREFIX|g;s|{EMP_SCRIPTS_DIR}|$EMP_SCRIPTS_DIR|g;s|{EMP_ASSETS_ROOT_DIR}|$EMP_ASSETS_ROOT_DIR|g;s|{EMP_TFTPROOT_DIR}|$EMP_TFTPROOT_DIR|g;s|{EMP_PHP_FPM_RUN_SOCK}|$EMP_PHP_FPM_RUN_SOCK|g;s|{EMP_DRIVERS_BASE_DIR}|$EMP_DRIVERS_BASE_DIR|g;s|{EMP_WEBSERVER_DRIVERS_PATH_PREFIX}|$EMP_WEBSERVER_DRIVERS_PATH_PREFIX|g;" "$EMP_NGINX_DRIVERS_CONF_TEMPLATE" >> "$EMP_NGINX_CONF_FINAL"
